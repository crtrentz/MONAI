...
class ShiftIntensityd(MapTransform):
    ...
    def __init__(self, keys: Hashable, offset: float):
        """
        Args:
            keys (hashable items): keys of the corresponding items to be transformed.
                See also: :py:class:`monai.transforms.compose.MapTransform`
...
class ScaleIntensityd(MapTransform):
    ...
    def __init__(
        self, keys: Hashable, minv: float = 0.0, maxv: float = 1.0, factor: Optional[float] = None,
    ):
        """
        Args:
            keys (hashable items): keys of the corresponding items to be transformed.
...
class ThresholdIntensityd(MapTransform):
    ...
    def __init__(self, keys: Hashable, threshold: float, above: bool = True, cval: float = 0):
        super().__init__(keys)
...
class AdjustContrastd(MapTransform):
    ...
    def __init__(self, keys: Hashable, gamma: float):
        super().__init__(keys)
